...
public class CheckConstraintTozalash implements ApplicationRunner {
    ...
    /*
     * PostgreSQL constraint matnini shunday chiqaradi:
     *   CHECK (((holat)::text = ANY ((ARRAY['SOTILDI'::character varying, ...])::text[])))
     * E'tibor bering: ARRAY oldida QO'SHIMCHA qavs bor, shuning uchun
     * '= any (array[' shabloni MOS KELMAYDI. Faqat 'array[' + '::text' bo'yicha
     * qidiramiz va nomi '_check' bilan tugaganini olamiz — bu Hibernate'ning
     * enum uchun yaratgan CHECK'ining imzosi.
     */
    private static final String QIDIRUV = """
            select conrelid::regclass::text as jadval,
                   conname as nomi,
                   pg_get_constraintdef(oid) as tavsif
            from pg_constraint
            where contype = 'c'
              and connamespace = 'public'::regnamespace
              and conname like '%\\_check'
              and lower(pg_get_constraintdef(oid)) like '%array[%'
        ...
        try {
            ...
            for (Map<String, Object> qator : royxat) {
                ...
                try {
                    ...
                    log.info("Eski enum CHECK constraint o'chirildi: {}.{}  ({})",
                            jadval, nomi, qator.get("tavsif"));
                } catch (Exception e) {
